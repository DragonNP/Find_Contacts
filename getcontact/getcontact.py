...
class GetContactAPI:
    ...
    def get_information_by_phone(self, phone):
        ...
        return dict(**result_name, **result_tags, **{'remainCount': self.remain_count})

    def print_information_by_phone(self, phone):
        data = self.get_information_by_phone(phone)
        return self._print_beauty_output(data)
    ...
    def _print_beauty_output(self, data):
        ...
        if 'tags' in data.keys() and data['tags']:
            ...
            for tag in data['tags']:
                result += '\t ' + tag + '\n'
                print('\t', tag)
        result += '\nRemain count: ' + str(data['remainCount'])
        print('Remain count:', data['remainCount'])
